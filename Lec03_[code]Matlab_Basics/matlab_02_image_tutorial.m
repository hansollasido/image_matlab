...
%% Read in an image 
%
im1 = imread('sunset.jpg');  %% Give full path name in the string
...
close(2)
figure(1)
subplot(2,2,3) % 'subplot 121' is also legal syntax
imshow(im1)
%subplot 221, imshow(grIm1);
subplot (2,2,1), imshow(grIm1);
...
%
imwrite(tIm1, 'threshImage.png');  %% Saved as 'jpg'
...
%
subplot (1,3,1), imshow(grIm1); title('Grayscale Image')
subplot (1,3,2), surf(gaussFilt); title('Gaussian Filter')
subplot (1,3,3), imshow(convIm); title('Gaussian Filtered Image')

%print(1, '-djpeg90', '-r150', 'result.jpg');
...
subplot (1,2,1), imshow(subIm);
subplot (1,2,2), imshow(subIm_col);
